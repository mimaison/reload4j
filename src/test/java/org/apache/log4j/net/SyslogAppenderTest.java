...
public class SyslogAppenderTest extends TestCase {
	...
	String expected = "kern user mail daemon auth syslog lpr news "
		+ "uucp cron authpriv ftp local0 local1 local2 local3 " + "local4 local5 local6 local7 ";
	...
	String[] names = new String[] { "kErn", "usEr", "MaIL", "daemOn", "auTh", "syslOg", "lPr", "newS", "Uucp",
		"croN", "authprIv", "ftP" };
	...
	String[] names = new String[] { "lOcal0", "LOCAL1", "loCal2", "locAl3", "locaL4", "local5", "LOCal6",
		"loCAL7" };
    ...
    /**
     * Test setFacility with null. Should have no effect.
     */
    ...
    /**
     * Test setFacility with bogus value. Should reset to user.
     */
    ...
    /**
     * Tests that append method drops messages below threshold. Can't reach
     * isSevereAsThreshold call in SyslogAppender.append since it is checked in
	...
	logger.info("Should not be logged by SyslogAppenderTest.testAppendBelowThreshold.");
    }
	...
	Logger logger = Logger.getRootLogger();
	logger.addAppender(appender);
	logger.info("Should not be logged by SyslogAppenderTest.testAppendNoHost.");
	assertEquals(1, errorHandler.size());

	//
	// Appender is misspelled in implementation
	//
	assertEquals("No syslog host is set for SyslogAppedender named \"foo\".", errorHandler.getMessage(0));
    }
    ...
    /**
     * Tests SyslogAppender with IPv6 address enclosed in square brackets followed
     * by port specification.
     */
    ...
    /**
     * Tests SyslogAppender with host name enclosed in square brackets followed by
     * port specification.
     */
    ...
    /**
     * Tests that syslog packets do not exceed 1024 bytes. See bug 42087.
     * 
     * @throws Exception if exception during test.
     */
